...
class DashboardDemoSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $netflix = Service::create([
            ...
            'stock_service'       => 0, // akan di-sync dari tracker
            'price_service'       => 35000,
        ]);
        ...
        // Tracker dimulai dari 0, dihitung dari movement pertama
        $stockTracker = [
            $netflix->id => 0,
            $spotify->id => 0,
            $canva->id   => 0,
        ...
        foreach ($movements as [$service, $type, $qty, $date]) {
            $stockBefore = $stockTracker[$service->id];

            $stockTracker[$service->id] = $type === 'in'
                ? $stockBefore + $qty
                : $stockBefore - $qty;
            ...
            StockService::create([
                ...
                'stock_after'  => $stockTracker[$service->id],
                'type'         => $type,
                'created_at'   => $date,
                'updated_at'   => $date,
            ]);
        ...
        // Sync stock_service di tabel services sesuai hasil akhir tracker
        foreach ($stockTracker as $serviceId => $finalStock) {
            Service::where('id', $serviceId)->update(['stock_service' => $finalStock]);
        }
    }
}